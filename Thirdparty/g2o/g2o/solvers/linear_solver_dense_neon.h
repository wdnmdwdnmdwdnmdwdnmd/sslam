#ifndef G2O_LINEAR_SOLVER_DENSE_NEON_H
#define G2O_LINEAR_SOLVER_DENSE_NEON_H

#include "../core/linear_solver.h"
#include "../core/batch_stats.h"
#include "../../../../lyh_fake_ldlt.h"  // Your NEON optimized LLT
...
namespace g2o {
  ...
  /**
   * \brief linear solver using NEON-optimized dense cholesky decomposition
   */
  template <typename MatrixType>
  class LinearSolverDenseNEON : public LinearSolver<MatrixType>
  {
    public:
      LinearSolverDenseNEON() :
        LinearSolver<MatrixType>(),
        _reset(true)
      {
        std::cout << "[LinearSolverDenseNEON] Initialized with NEON optimization!" << std::endl;
      }

      virtual ~LinearSolverDenseNEON()
      {
      }
      ...
      bool solve(const SparseBlockMatrix<MatrixType>& A, double* x, double* b)
      {
        ...
        // solving via NEON-optimized Cholesky decomposition
        Eigen::VectorXd::MapType xvec(x, m);
        Eigen::VectorXd::ConstMapType bvec(b, n);
        
        _cholesky_neon.compute(H);
        if (_cholesky_neon.isPositive()) {
          xvec = _cholesky_neon.solve(bvec);
          return true;
        }
        return false;
      }
    ...
    protected:
      bool _reset;
      Eigen::MatrixXd _H;
      lyh::FakeLDLT _cholesky_neon;  // Use your NEON-optimized LLT
