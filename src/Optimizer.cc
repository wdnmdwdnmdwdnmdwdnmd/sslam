...
// LYH: Add NEON-optimized dense solver
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    Eigen::Matrix4f Tcw_opt = pFrame->GetPose().matrix().cast<float>();
    std::cout << "[Pose after PnP]\n" << Tcw_opt << std::endl;
...
#ifdef USE_NEON_SOLVER
    std::cout << "[LocalBA] Using NEON-optimized Dense Solver (matrix size: " << num_OptKF * 6 << ")" << std::endl;
