...
void Tracking::Track()
{
    std::cout << "[Track] new frame id = " << mCurrentFrame.mnId << std::endl;
...
bool Tracking::TrackWithMotionModel()
{
    ...
    Eigen::Matrix4f Tcw = mCurrentFrame.GetPose().matrix().cast<float>();
    std::cout << "[Pose after motion model]\n" << Tcw << std::endl;
...
bool Tracking::TrackLocalMap()
{
    ...
    Eigen::Matrix4f Tcw_local = mCurrentFrame.GetPose().matrix().cast<float>();
    std::cout << "[Pose after local map]\n" << Tcw_local << std::endl;
